...
CREATE_ACCOUNT_LINK = f"{BASE_URL}/onboarding"
EDIT_PROFILE_LINK = f"{BASE_URL}/profile/edit"
...
def _placeholder_photo_url() -> str:
    if not settings.PLACEHOLDER_PHOTO_S3_KEY:
        return ""
...
def _build_actions_notification(performed_flags: list[int]) -> str:
    lines = [
        OPTION_NOTIFICATION_LINES[flag]
        for flag in OPTION_ORDER
...
async def _apply_options(session, snapshot: UserProfileSnapshot, flags: int) -> list[int]:
    ...
    if not user:
        return performed
    if flags & OPTION_HIDE_PHOTO and settings.PLACEHOLDER_PHOTO_S3_KEY:
...
async def _delete_user_data(session, user_id: int) -> None:
    ...
    await session.execute(
        delete(InstagramConnection).where(InstagramConnection.user_id == user_id)
    )
